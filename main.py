...
values = np.zeros(dtype=np.float32, shape=(20,20))
...
policy = np.zeros(dtype=np.int32, shape=(20,20))
...
def evaluate_policy():
    while True:
        ...
        state_ids = np.random.permutation(400)
        delta = 0
        for i in range(0, 400):
            state_id = state_ids[i]
            state_idx_i, state_idx_j = state_id // 20, state_id % 20
            ...
            # over night
            a = policy[cars_at_store_0, cars_at_store_1]
            cars_at_store_0 -= a
            cars_at_store_1 += a
            ...
            expected_reward_for_store_0_given_state_and_policy = np.einsum(
                'i,i->', probs_cars_requested_from_store_0, renting_reward_multiple * np.arange(0, cars_at_store_0 + 1)
                # noninclusive range!
            )
            expected_reward_for_store_1_given_state_and_policy = np.einsum(
                'i,i->', probs_cars_requested_from_store_1, renting_reward_multiple * np.arange(0, cars_at_store_1 + 1)
            )
            ...
            probs_cars_left_at_store_0 = probs_cars_requested_from_store_0[::-1]
            probs_cars_left_at_store_0[0] += 1.0 - np.sum(probs_cars_requested_from_store_0)

            probs_cars_left_at_store_1 = probs_cars_requested_from_store_1[::-1]
